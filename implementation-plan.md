...
### 1. プロジェクト基盤構築
- [ ] **1.1** TypeScript + Node.jsプロジェクトの初期化
  - package.jsonの作成
...
### デプロイメント戦略
- **ZIPファイル直接アップロード方式**を採用
- AWS CLIを使用したシンプルなデプロイ
- 手動IAM設定による初期セットアップ
- スクリプトによる自動ビルド・デプロイ
...
### ZIPファイルアップロード方式の利点
1. **シンプル**: CloudFormationの複雑な設定が不要
2. **高速**: 直接アップロードで素早いデプロイ
3. **透明性**: 何がデプロイされるかが明確
4. **デバッグ容易**: 問題発生時の原因特定が簡単

### 実装予定スクリプト
- `build.sh` / `build.bat`: TypeScriptコンパイル + 依存関係インストール + ZIPパッケージ生成
- `deploy.sh` / `deploy.bat`: AWS CLIを使ったLambda関数のアップロード
- `setup.md`: 初回セットアップ手順（IAM、EventBridge設定）
